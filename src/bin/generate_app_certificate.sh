...
printUsage() {
    echo "Usage: $PROG ENTITY-ID ENDPOINT-URL DST_PATH"
    echo ""
    echo "Example:"
...
BASEURL="$2"
if [ -z "$BASEURL" ]; then
    echo "$PROG: The URL to the MellonEndpointDST_PATH is required." >&2
    exit 1
fi
#echo "DEBUG 2"
DST_PATH="$3"
...
#echo "DEBUG 4"
OUTFILE="$DST_PATH$(echo "$ENTITYID" | sed 's/[^0-9A-Za-z.-]/_/g' | sed 's/__*/_/g')"
#echo "DEBUG 5"
